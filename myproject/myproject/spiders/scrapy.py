...
class QuotesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        c = conn.cursor()
        c.execute("CREATE TABLE test11 (id serial PRIMARY KEY, title varchar, herf varchar, image varchar, rating varchar, price varchar, location varchar, amenities text);")
        # conn.commit()
        for quote in response.css('div.soom'):
            if item < 12:
                ...
                if img is not None:
                    c.execute("INSERT INTO test11 (title, herf, image, rating, price, location, amenities) VALUES (%s, %s, %s, %s, %s, %s, %s)", (title, herf, img, rating, price, location, amenities))
                    conn.commit()
                    # c.close()
                    # conn.close()
                elif img is None:
